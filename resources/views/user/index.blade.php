...
<div class="main_area_access_user_auth">
    <article class="main_area_access_user_auth_content">
        <header>
            <div class="main_login_logo">Area de Login</div>
            <h1>Olá, loge-se abaixo!</h1>
        </header>
        <form action="{{ route('user.login.do') }}" autocomplete="off" method="post" name="login">
            @csrf
            ...
            <button type="submit" class="icon-check-square-o" >Entrar</button>
        </form>
        <div class="main_login_register">
            <small>Ainda não possui login?</small>
